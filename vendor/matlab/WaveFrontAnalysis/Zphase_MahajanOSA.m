function [phasemap PixelsInPupil]=Zphase_Mahajan(c,PARAMS, PupilFieldSize)
...
phasemap = zeros(PARAMS.PixelDimension,PARAMS.PixelDimension);

sizeoffield=PupilFieldSize;

PixelsInPupil = 0; %initialize
%*********************************************************************************************
...
for ny = 1:PARAMS.PixelDimension

	for nx = 1:PARAMS.PixelDimension

		xpos = ((nx-1)*(sizeoffield/PARAMS.PixelDimension)-(sizeoffield/2));
		ypos = ((ny-1)*(sizeoffield/PARAMS.PixelDimension)-(sizeoffield/2));

      [angle norm_radius]=cart2pol(xpos,ypos);
      norm_radius=norm_radius/(PARAMS.PupilSize/2);
      r=norm_radius;
	...
	if norm_radius > 1;
         ...
         SCfactor=B+A*10^(-p*(PARAMS.PupilSize^2)*d^2);
      
      	 phasemap(nx,ny) = SCfactor * exp(-i * 2 * 3.1416 * phase/PARAMS.ImagingWavelength);
         PixelsInPupil = PixelsInPupil + 1;
		end
	end
end
